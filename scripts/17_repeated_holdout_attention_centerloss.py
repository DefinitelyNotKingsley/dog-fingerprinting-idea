...
OUT_ALL = RESULTS_DIR / "voxblink_attention_centerloss_all_results.csv"
OUT_SUMMARY = RESULTS_DIR / "voxblink_attention_centerloss_summary.csv"
...
class AttentionFusionMLP(nn.Module):
    def __init__(self, input_dim, embed_dim=256):
        ...
        # Current fusion3 is usually:
        # Dog2Vec 768 + MFCC 80 + ECAPA 192 = 1040
        # If your input is older 848-D, this becomes Dog2Vec 768 + MFCC 80.
        if input_dim == 1040:
            self.block_dims = [768, 80, 192]
        elif input_dim == 848:
            self.block_dims = [768, 80]
        else:
            # fallback: treat whole vector as one block
            self.block_dims = [input_dim]
    ...
    def forward(self, x):
        chunks = torch.split(x, self.block_dims, dim=1)

        projected = []
        attn_scores = []

        for chunk, proj in zip(chunks, self.proj_layers):
            h = proj(chunk)
            projected.append(h)
            attn_scores.append(self.attention(h))

        H = torch.stack(projected, dim=1)
        A = torch.stack(attn_scores, dim=1)
...
def train_model(X_train, y_train, seed):
    ...
    model = AttentionFusionMLP(input_dim=X_train.shape[1], embed_dim=EMBED_DIM).to(DEVICE)
...
def main():
    ...
    print("\nVoxBlink-style + Attention Fusion + Center Loss summary by enrollment size:")
