...
X_TRAIN_PATH = RESULTS_DIR / "speechbrain_ecapa_train_embeddings.npy"
X_TEST_PATH = RESULTS_DIR / "speechbrain_ecapa_test_embeddings.npy"
Y_TRAIN_PATH = DATA_DIR / "train_labels.npy"
Y_TEST_PATH = DATA_DIR / "test_labels.npy"

OUT_ALL = RESULTS_DIR / "ecapa_centerloss_all_results.csv"
OUT_SUMMARY = RESULTS_DIR / "ecapa_centerloss_summary.csv"
...
class ECAPAProjection(nn.Module):
    def __init__(self, input_dim=192, embed_dim=256):
        super().__init__()
...
def train_model(X_train, y_train, seed):
    ...
    model = ECAPAProjection(input_dim=X_train.shape[1], embed_dim=EMBED_DIM).to(DEVICE)
...
def main():
    ...
    print("\nECAPA-only + Center Loss VoxBlink-style summary by enrollment size:")
