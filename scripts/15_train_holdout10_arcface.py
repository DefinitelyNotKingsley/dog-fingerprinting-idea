...
def main():
    ...
    X_train = np.load(DATA_DIR / "holdout10/known_train_X.npy").astype("float32")
    X_test = np.load(DATA_DIR / "holdout10/known_test_X.npy").astype("float32")
    y_train = np.load(DATA_DIR / "holdout10/known_train_y.npy", allow_pickle=True)
    y_test = np.load(DATA_DIR / "holdout10/known_test_y.npy", allow_pickle=True)
    ...
    torch.save(model.state_dict(), MODEL_DIR / "holdout10_fusion3_mlp_arcface.pt")
    torch.save(head.state_dict(), MODEL_DIR / "arcface_head.pt")

    np.save(RESULTS_DIR / "holdout10_train_embeddings.npy", train_emb)
    np.save(RESULTS_DIR / "holdout10_known_test_embeddings.npy", test_emb)
